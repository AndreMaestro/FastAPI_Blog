...
class DeletePostImagesUseCase:
    ...
    async def execute(self, post_id: int,
        ...
        async with self._database.session() as session:
            post = await self._post_repo.get_by_id(session=session, id=post_id)
            if not post:
                raise PostNotFoundByIdException(post_id)
            images = await self._image_repo.get_images(session, "post", post_id)
            if images:
                images_paths = [img.file_path for img in images]
                await self._image_repo.delete_images(session, "post", post_id)
